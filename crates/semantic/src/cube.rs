...
//!
//! Cube's row-level security model is uniquely convenient: the entire signed-JWT
//! payload *is* the security context, and Cube enforces RLS server-side via
//! `queryRewrite` / `COMPILE_CONTEXT`. See [`crate::dbt`] for the contrast — dbt
//! has no equivalent, so RLS there is injected as explicit `where` filters.
